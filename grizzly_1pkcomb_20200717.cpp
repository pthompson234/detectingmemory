...
{
  //The combination model is the most complicated. We have to include everything!
  ...
  DATA_MATRIX( res_casesfit ); // for all data not just what's being fit
  DATA_MATRIX( res_casesall );
  DATA_MATRIX( res_controls ); // for controls
  ...
  PARAMETER( sd_tau ); // variation in memory-based movement cycle; length n_peaks
  ...
  for (int i = 0; i < n_cases; i++) {
    ...
    for (int tau = 1; tau <= t; tau++) {
      ...
      res_coefs(tau - 1) = (res_casesall(t - tau) * beta_res).sum() + threshold;
    }
    dist_cases(i) = (dnorms_sub * res_coefs * exp(-exp_alpha * dist_vars_cs)).sum() * (1.0 / sumNorm);
    ...
    for (int j = 0; j < n_simmed; j++) {
      ...
      dist_controls(i * n_simmed + j) = (dnorms_sub * res_coefs * exp(-exp_alpha * dist_vars_ct)).sum() * (1.0 / sumNorm);
    }
  }
  ...
  vector<Type> cs_vector = dist_coef * dist_cases + res_casesfit * beta_res;
  vector<Type> sums(n_cases);
  
  vector<Type> res_controls_all = res_controls * beta_res; // do all at once then subset
  
  for( int i = 0; i < n_cases; i++) {
    vector<Type> arr = dist_coef * dist_controls.segment(i * n_simmed, n_simmed) + res_controls_all.segment(i * n_simmed, n_simmed);
